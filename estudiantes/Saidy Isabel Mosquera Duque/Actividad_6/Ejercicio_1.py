...
for c in lista_revistas:
     tipo = c.split("; ")[0]
     if ("Artículos academicos".lower()) in tipo.lower():
          bibliografia["revista"] = c.split("; ")[3]
          bibliografia["volumen"] = c.split("; ")[6]
          bibliografia["numero"] = c.split("; ")[7]


     print(bibliografia)

 # NO ENTIENDO POR QUE NO ME RECONOCE SINO LA ULTIMA LINEA. 
 # PORQUE NO RECONOCE LAS NUEVAS CLAVES     
...
for clave, valor in bibliografia.items():
        ...
        Autor: {bibliografia[clave]["autor"][0]["nombre"]}, {bibliografia[clave]["autor"][0]["apellido"]}
        Título: {bibliografia[clave]["titulo"]}
        Revista: {bibliografia["revista"]}
        Fecha: {bibliografia[clave]["fecha"]}
        Lugar: {bibliografia[clave]["lugar"]}
        Volumen: {bibliografia["volumen"]}
        No: {bibliografia["numero"]}
