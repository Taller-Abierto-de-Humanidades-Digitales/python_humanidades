...
#EJERCICIO 1: FUNCIÓN QUE EXTRAE AUTORES EN UNA LISTA-------------------------------------------------------------------------------------------

def autores(katalog: dict)->list:
    """ Retorna una lista con el nombre de los autores del catálogo.
    ...
    for elemento in katalog.keys():
        autoreslist.append(f"{elemento}")
    return autoreslist

#print(autores(biografias_boom))


#EJERCICIO 2: FUNCIONES QUE RETORNAN FECHAS-----------------------------------------------------------------------------------------------------
#-------------FUNCIÓN QUE EXTRAE TODAS FECHAS DE CUALQUIER STRING-------------------------------------------------------------------------------

def extraer_fechas(texto: str)->list:
    """Extrae todas los años que aparecen en un texto.
...
#-------------FUNCIÓN QUE EXTRAE TODAS FECHAS DE NACIMIENTO DE LOS AUTORES----------------------------------------------------------------------

def fecha_nacimiento(katalog: dict,nompellido: str)->int:
    """ Retorna una lista con las fechas de nacimiento de los autores del catálogo.
     ...
     katalog: dict
        El catálogo en el que se desea buscar
     nompellido: str
        Nombre y apellido del autor del que se desea conocer su fecha de nacimiento
     
     Retorna
     ----------
     nacimiento: int
        La fecha de nacimiento del autor indicado"""
    nacimiento=0
    resultado=[]
    if len(nompellido.strip().split(" ")) < 2:
        return "Por favor, indique un nombre completo"
    else:
        for y in katalog.keys():
            if nompellido.strip().lower() in y.lower() or y.lower() in nompellido.strip().lower():
                  resultado.append(y)  
        if resultado:
            return int(min(extraer_fechas(katalog[resultado[0]]["bio"])))
        else:        
             return f"Parece que no se tiene información de {nompellido}. No olvide utilizar los acentos y ortografía adecuada. O puede no existir ese autor en el catálogo. Verifique que esté en el orden Nombre y Apellido."
                    
#print(fecha_nacimiento(biografias_boom," julio cortázar "))
#print(isinstance(fecha_nacimiento(biografias_boom, "julio cortázar "), int))

#EJERCICIO 3:
#-------------EDAD AL PUBLICAR PRIMER OBRA------------------------------------------------------------------------------------------------------

def calcular_edad(katalog, nombre_autor) -> int:
    """ Retorna una lista con la edad a la que un autor publicó su obra principal.
     ...
     katalog: dict
        El catálogo en el que se desea buscar
     nombre_autor: str
        Nombre y apellido del autor del que se desea la información
     
     Retorna
     ----------
     edad_publicacion: int
       Edad a la que un autor publicó su obra principal"""
    edad_pub=0
    x=0
    if len(nombre_autor.strip().split(" ")) < 2:
        return "Por favor, indique un nombre completo"
    else:
        for y in katalog.keys():
            if nombre_autor.strip().lower() in y.lower() or y.lower() in nombre_autor.strip().lower(): #Verificación de nombre                  
                    x=int(katalog[y]['fecha_publicacion'])
        if isinstance(fecha_nacimiento(katalog, nombre_autor), int):
            edad_pub=x-fecha_nacimiento(katalog, nombre_autor)
            return edad_pub
        else:
            return f"{fecha_nacimiento(katalog, nombre_autor)}"
                    
#print(calcular_edad(biografias_boom, "julio cortázar "))

#EJERCICIO 3:
#-------------EDAD PROMEDIO AL PUBLICAR PRIMER OBRA---------------------------------------------------------------------------------------------
def edad_pub_promedio(katalog: dict)->int:
    """ Retorna  el número entero más cercano el valor promedio de las edades a las que los autores del catálogo publicaron su obra principal
     ...
     ----------
     edad_prom: int
       Menor número entero igual o mayor que el valor promedio de las edades"""
    import math
    lista_autores=autores(katalog)
    edades=[]
    prom=0
    for n in lista_autores:
        edades.append(calcular_edad(katalog, f"{n}"))
    if edades != []:
         prom=sum(edades)/len(edades)
         validador=math.ceil(prom/0.5)*0.5
         if prom >= validador:
              return math.ceil(prom)
         else:
              return math.floor(prom)
    else:
         return "No se puede calcular el promedio. Por favor, verifique que se pueda calcular la edad de publicación y que el catálogo ingresado tenga los datos necesarios"

#print(edad_pub_promedio(biografias_boom))

#EJERCICIO 4:
#-------------EXTRAE FECHA DE DEFUNCIÓN (SI APLICA)---------------------------------------------------------------------------------------------
def fecha_defuncion(katalog: dict, nompellido: str)->int:
     """ Retorna la fecha de defunción de un autor determinado.
     ...
     katalog: dict
        El catálogo en el que se desea buscar
     nompellido: str
        Nombre del autor de quien se desea la fecha de muerte     

     Retorna
     ----------
     defuncion: int
       Fecha de defunción del autor"""
     fechas_int=[]
     if len(nompellido.strip().split(" ")) < 2:
        return "Por favor, indique un nombre completo"
     else:
        for y in katalog.keys():
            if nompellido.strip().lower() in y.lower() and katalog[y]['vivo']==False:                       
                defuncion=int(max(extraer_fechas(katalog[y]['bio'])))
                return defuncion
            elif nompellido.strip().lower() in y.lower() and katalog[y]['vivo']==True:
                return None
        else:
            return f"El autor \"{nompellido}\" no se encuentra en el catálogo. Verifique que esté en el orden Nombre y Apellido."
          
#print(fecha_defuncion(biografias_boom, " gabriel garcía "))

#EJERCICIO 4:
#-------------EXTRAE EDAD DE DEFUNCIÓN (SI APLICA)---------------------------------------------------------------------------------------------

def edad_defuncion(katalog: dict, nompellido: str)->int:
     """ Retorna la edad de defunción de un autor determinado (si aplica).
     ...
     katalog: dict
        El catálogo en el que se desea buscar
     nompellido: str
        Nombre del autor de quien se desea la edad de muerte     

     Retorna
     ----------
     defuncion: int
       Edad de defunción del autor"""
     if fecha_defuncion(katalog, nompellido) is not None:
            return fecha_defuncion(katalog, nompellido) - fecha_nacimiento(katalog, nompellido)
     else:
             return f"El autor {nompellido.strip()} sigue vivo"   

#print(edad_defuncion(biografias_boom, "gabriel garcía "))

#EJERCICIO 5: DETERMINA EL O LOS MÁS LONGEVOS DE LOS AUTORES------------------------------------------------------------------------------------

def longevo(katalog: dict):
     """ Retorna el autor más longevo de un catálogo (sin importar si está muerto o vivo).
     ...
     ----------
     longevo: int
       Autor que tenga la mayor edad"""
     import datetime
     año_actual= int(datetime.date.today().year)
     lista_edades=[]
     lista_nedades=[]
     for n in autores(katalog):
          if fecha_defuncion(katalog, n) is not None:
               lista_edades.append(edad_defuncion(katalog, n))
               lista_nedades.append([n, edad_defuncion(katalog, n)])
          else:
               lista_edades.append(año_actual - fecha_nacimiento(katalog, n) )
               lista_nedades.append([n, año_actual - fecha_nacimiento(katalog, n), "vivo"])
     if lista_edades != []:
          maximum=max(lista_edades)
          maximos=[i for i in lista_nedades if i[1]==maximum]
          return maximos

#print(longevo(biografias_boom))

#EJERCICIO 6: FUNCIÓN RECURSIVA-----------------------------------------------------------------------------------------------------------------
def main(katalog: dict)->None:
    """
    ...
    if len(longevo_autores_boom) >= 2:
         ...
         alles=", ".join(nombres)
         print(f"Los autores más longevos de los autores del Boom Latinoamericano fueron {alles} quienes vivieron {longevo_autores_boom[1][1]} años.")
    else:
         print(f"El autor más longevo de los autores_boom del Boom Latinoamericano fue {longevo_autores_boom[0][0]} quien vivió {longevo_autores_boom[0][1]} años")
